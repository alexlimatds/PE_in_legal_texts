...
class SingleSC_BERT(torch.nn.Module):
    ...
    """
    def __init__(self, encoder_id, n_classes, dropout_rate, embedding_dim, use_mlp, n_hidden_mlp=100, norm_layer=False):
        '''
        ...
        sequence_layers = []
        if norm_layer:
            sequence_layers.append(torch.nn.LayerNorm(embedding_dim))
        if use_mlp:
