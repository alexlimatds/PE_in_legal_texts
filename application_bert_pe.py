...
import pandas as pd
import numpy as np
import csv, random, torch, models_bert_pe, transformers, time
...
def evaluate_BERT(train_params):
    ...
    max_sentence_len = train_params['max_sentence_len']
    k_mvpe = train_params['mvpe_step']
    ...
    if train_params.get('n_documents') is not None: # used in tests to speed up the train procedure
        ...
        ds_train = models_bert_pe.Single_SC_PE_Dataset(temp_docs_train, labels_to_idx, tokenizer, embedding_dim, max_sentence_len, k_mvpe)
        ds_test = models_bert_pe.Single_SC_PE_Dataset(temp_docs_test, labels_to_idx, tokenizer, embedding_dim, max_sentence_len, k_mvpe)
    else:
        ds_train = models_bert_pe.Single_SC_PE_Dataset(dic_docs_train, labels_to_idx, tokenizer, embedding_dim, max_sentence_len, k_mvpe)
        ds_test = models_bert_pe.Single_SC_PE_Dataset(dic_docs_test, labels_to_idx, tokenizer, embedding_dim, max_sentence_len, k_mvpe)
    ...
    for i, seed_val in enumerate(seeds):
        ...
        # train
        iteration_metrics, cm, train_time = models_bert_pe.fit(train_params, ds_train, ds_test, device)
...
def save_report(
    ...
    report = (
        ...
        f'Features combination: {train_params["combination"]}\n'
        f'Batch size: {train_params["batch_size"]}\n'
        f'Dropout rate: {train_params["dropout_rate"]}\n'
        f'Learning rate: {train_params["learning_rate"]}\n'
        f'mvPE step: {train_params["mvpe_step"]}\n'
        f'Max sentence length: {train_params["max_sentence_len"]}\n'
