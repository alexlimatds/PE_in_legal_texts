...
class ShulayevaEncoder:
    ...
    def train(self, dic_docs):        
        ...
        # TF-IDF Unigrams
        self.tfidf_unigrams = TfidfVectorizer(
            tokenizer=self.spacy_tokenizer, 
            ngram_range=(1, 1), 
            max_features=2000
        )
        self.tfidf_unigrams.fit(sentences)
        
    def get_features_info(self):
        V_pos = len(self.tfidf_pos_tags.vocabulary_)
        V_pairs = len(self.tfidf_pairs.vocabulary_)
        V_unigrams = len(self.tfidf_unigrams.vocabulary_)
        embedding_dim = V_pos + V_pairs + V_unigrams + 2 # 1 => sentence lenght, 1 => sentence position
        return f'embedding dim: {embedding_dim} => PoS tag features: {V_pos}, Dependecy pair features: {V_pairs}, Unigram features: {V_unigrams}'
    ...
    def encode(self, dic_docs):
        ...
        tfidf_features = self.tfidf_unigrams.transform(sentences)
        len_features = np.array([len(s) for s in sentences]).reshape((-1,1)) # sentences lengths
        return self.merge_features(tfidf_features, positions, len_features, pos_features, pair_features), targets


if __name__ == "__main__":
    model_reference = 'Shulayeva'
    #application_sparse.evaluate_sparse(ShulayevaEncoder(), model_reference, n_docs=10) # for tests
    application_sparse.evaluate_sparse(ShulayevaEncoder(), model_reference)
